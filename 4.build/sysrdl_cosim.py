...
###################################################################
# Python script to generate the openPCIE HAL co-sim adapter layer
#
# Copyright (C) 2026 Chili.CHIPS*ba
#
# Walks the elaborated SystemRDL model of 4.build/csr_build/csr.rdl and
# emits a C++ class hierarchy that mirrors the RDL hierarchy. Two flavours
# come out of the same model and present the SAME API to the application:
#
#   without -c : csr_hw.h    -- plain pointer dereferences through the
#                               "peakrdl c-header" structs, for code running
#                               on the picorv32 (and on the rv32 ISS)
#   with    -c : csr_cosim.h -- VProc VWrite/VRead transactions, for code
#                               running natively in the co-simulation
#
# options:
...
class Listener(RDLListener) :
  ...
  def __gen_header(self) :
    ...
    print("// Common openPCIE HAL header for both platform and co-simulation\n//");
    ...
    if self.cosim :
      ...
      print("#define PCIEMAIN                VUserMain" + str(self.vpnode))
...
def processCmdLine():
    ...
    parser.add_argument('-C', '--clk_period',  dest='clkperiod', default=16000,       action='store',      help='Specify the VProc soc_cpu clock period in ps (cosim only)')
